...
ihme_datafile <- fread("dataset_generation/files/data/IHME-GBD_2019_DATA-d1ce1140-1/IHME-GBD_2019_DATA-d1ce1140-1.csv")
...
# Data import
diabetes_mortality <- ihme_datafile %>% 
  filter(measure_id == 1 & cause_id == 587) %>% 
...
# Data import
diabetes <- ihme_datafile %>% 
  filter(measure_id == 5 & cause_id == 587) %>% 
  select(location_name, year, val) %>% 
...
# Data import
hypertension_mortality <- ihme_datafile %>% 
  filter(measure_id == 1 & cause_id == 498) %>% 
  select(location_name, year, val) %>% 
...
# Data import
hypertension <- ihme_datafile %>% 
  filter(measure_id == 5 & cause_id == 498) %>% 
  select(location_name, year, val) %>% 
